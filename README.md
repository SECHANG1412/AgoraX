...
프론트엔드 회귀 테스트는 인증·관리자 경로 접근 제어, 관리자 대시보드의 API 부분 실패 처리, 토픽 목록의 비동기 요청·페이지네이션, 토픽 생성의 중복 제출 방지, 토픽 상세 조회·투표 흐름을 API 대역으로 검증합니다. CI에서는 Vitest, lint, TypeScript 타입 검사와 production build를 실행합니다.
